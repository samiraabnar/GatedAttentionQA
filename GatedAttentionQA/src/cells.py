...
class LSTMCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope("BasicLSTMCell"):
      ...
      concat = tf.contrib.layers.fully_connected(inputs=[inputs, h, self.c], num_outputs=4 * self._num_units,
                                        biases_initializer=tf.constant_initializer(0.0), activation_fn=None)
      # linear([inputs, h, self.c], 4 * self._num_units, True)
...
class MultiRNNCellWithSkipConn(tf.nn.rnn_cell.RNNCell):
